# gateway-http-skill-outbound-audit

## Purpose

定义 skill-gateway 在 **`gateway_outbound_audit_logs`** 中，对 **HTTP 外呼**（`outbound_kind` 为 HTTP、且与 Skill 代理相关）应支持的**增强**持久化：扩展 Skill 主键、agent 提交的 **API 代理原始参数**、外呼真实报文及 **HTTP 响应**，以便运维按 Skill 维度排障与合规审计；敏感信息遵守项目既有脱敏与截断策略。
