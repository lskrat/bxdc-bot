...
The system SHALL detect a `/<skill-name>` prefix at the start of the `instruction` field of `POST /agent/run` when the feature is enabled, and extract the skill name + remainder text.
...
#### Scenario: Feature enabled, slash prefix present
- **WHEN** `AGENT_SLASH_SKILL_INVOCATION=true` AND `instruction.trim()` starts with `/<token>` followed by whitespace AND `<token>` matches (case-insensitive, trimmed) the `name` of a skill in the conversation's enabled-skill list
- **THEN** the controller identifies `<token>` as the forced skill and treats the remainder as natural-language arguments

#### Scenario: Feature enabled, no slash prefix
- **WHEN** `AGENT_SLASH_SKILL_INVOCATION=true` AND `instruction` does NOT start with `/`
...
#### Scenario: Feature enabled, slash prefix but unknown skill
- **WHEN** `AGENT_SLASH_SKILL_INVOCATION=true` AND `instruction` starts with `/<token> ` AND `<token>` does NOT match any conversation-enabled skill name
...
#### Scenario: Slash without args
- **WHEN** instruction is `/测试时间` (no trailing text)
...
The frontend MessageInput component SHALL, when `AGENT_SLASH_SKILL_INVOCATION` (passed through a frontend-accessible config) is enabled, show an inline skill picker as soon as the user types `/` at the start of the input field.

#### Scenario: Trigger on leading `/`
- **WHEN** the input value starts with `/` (no leading whitespace)
...
#### Scenario: Filter by typed text after `/`
- **WHEN** the user has typed `/测` (or `/te`, etc.) in the input
- **THEN** the picker filters its list to skills whose `name` (case-insensitive) starts with or contains the typed token after `/`

#### Scenario: Selecting a skill
- **WHEN** the user clicks or keyboard-selects an entry in the picker
- **THEN** the input value is rewritten to `/<skill-name> ` (trailing space) so the user can continue typing arguments

#### Scenario: Dismiss picker
- **WHEN** the user clicks outside the picker, presses Escape, or deletes the leading `/`
