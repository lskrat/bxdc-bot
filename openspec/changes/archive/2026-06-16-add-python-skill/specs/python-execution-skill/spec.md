...
CONFIG-mode extended skills with canonical `kind=python` enable the LLM to invoke a third-party Python sandbox service via the Gateway. The LLM-supplied runtime parameters are validated against the sandbox row's `service_params` JSON Schema, then forwarded as the outbound request body; the LLM has no awareness of `kind=python` and uses the same outer-`payload` wrapper as `api` / `ssh` / `template` extension skills.
