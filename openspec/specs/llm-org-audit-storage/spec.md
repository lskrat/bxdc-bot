# llm-org-audit-storage Specification

## Purpose

由 skill-gateway 接收 agent-core 上报的 LLM 原始 HTTP 审计事件并写入关系型数据库，便于集中检索与按用户维度治理；agent-core 不直连该数据库。
...
- **THEN** 系统 SHALL 将记录写入 `llm_http_audit_logs`（或设计文档中最终表名）对应行，且每条记录 SHALL 包含数据库生成的 **记录时间**（`recorded_at` 或等价列）以及请求中给出的 **用户标识**（`user_id`，允许为空时存 NULL）
