...
系统 SHALL 将生成出的 API skill 通过现有 SkillGateway 持久化接口写入数据库，并保证写入后的记录能够被当前动态加载机制识别。
...
- **THEN** 保存结果 MUST 使用当前 `skills` 表标准字段
- **THEN** 保存后的 skill MUST 能通过 `GET /api/skills` 查询到

#### Scenario: Update existing generated skill with same target name
- **WHEN** 系统检测到待创建的 skill 名称已存在且用户选择覆盖或更新
- **THEN** 系统 MUST 更新现有 skill 配置而不是创建重复记录
