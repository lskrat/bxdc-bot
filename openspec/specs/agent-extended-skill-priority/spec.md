# agent-extended-skill-priority

## Purpose

规定 Agent Core 在同时存在 SkillGateway 扩展 Skill 与同类内置工具时的路由优先级与系统提示可见性。
...
Agent Core SHALL 在注入的系统/任务级策略文本中包含 **清晰可读** 的「扩展 Skill 优先」规则（见 design 中的 `[Extended skill routing]` 类段落），使模型在单次 run 内稳定服从上述优先级；规则 SHALL 与 SkillGateway 当前为用户加载的工具列表一致（不因未加载的 Skill 而虚构工具名）。
