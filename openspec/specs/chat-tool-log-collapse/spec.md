# chat-tool-log-collapse Specification

## Purpose

聊天页 assistant 消息的工具调用列表与确认卡片统一的折叠展示机制。折叠块的展开/收起由运行状态自动控制（运行中/流式推送期间展开，全部完成后收起），用户手动操作仅在非 pending 状态下介入，以避免工具调用日志把 LLM 文本回复推离视口。
...
#### Scenario: 流式推送期间保持展开
- **WHEN** 该消息是当前会话最新一条 assistant 消息且 SSE 流仍在推送（`isThinking`）
- **THEN** 折叠块 MUST 保持展开，即使工具逐个到达的间隙中暂时没有 `running` 状态的 tool
- **AND** 系统 MUST NOT 在工具到达间隙触发收起（避免闪烁）

#### Scenario: 全部完成后自动收起
- **WHEN** SSE 流结束、消息的所有 toolInvocations 状态均为 `completed` 或 `failed`，且无 `pending` 状态的确认卡片
- **THEN** 折叠块 MUST 自动收起
