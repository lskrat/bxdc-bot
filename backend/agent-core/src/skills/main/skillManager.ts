...
import { SqliteStore, cpRecursiveSync, getElectronNodeRuntimePath, appendPythonRuntimeToEnv } from './deps';
...
const downloadGithubArchive = async (
    ...
    archiveUrlCandidates.push(
      {
        ...
        headers: { 'User-Agent': 'BXDC.bot Skill Downloader' },
      },
      {
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'BXDC.bot Skill Downloader',
...
export class SkillManager {
          ...
          if (githubSource) {
            try {
              ...
              if (errno === 'ENOENT' && process.platform === 'win32') {
                throw new Error(
                  'Git executable not found. Please install Git for Windows or reinstall BXDC.bot with bundled PortableGit.'
            ...
            throw new Error('Git executable not found. Please install Git for Windows or reinstall BXDC.bot with bundled PortableGit.');
          } else {
            throw error;
          }
        }
  ...
  private getBundledSkillsRoot(): string {
    ...
    // In development, this file is compiled to dist-electron/skills/main/.
