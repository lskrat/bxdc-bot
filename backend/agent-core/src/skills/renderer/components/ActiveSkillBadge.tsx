...
import { toggleActiveSkill, clearActiveSkills } from '../skillSlice';
import { skillI18n as i18nService } from '../i18n';
import { Skill } from '../../shared/types';

const ActiveSkillBadge: React.FC = () => {
  const dispatch = useDispatch();
  const activeSkillIds = useSelector((state: any) => state.skill.activeSkillIds as string[]);
  const skills = useSelector((state: any) => state.skill.skills as Skill[]);
