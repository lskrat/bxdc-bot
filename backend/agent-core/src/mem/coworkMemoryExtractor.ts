// Extracted from src/main/libs/coworkMemoryExtractor.ts
...
const SHORT_FACT_SIGNAL_RE = /(我叫|我是|我的名字是|我名字是|名字叫|我的生日是|我生日是|我的出生日期是|我出生于|我生于|我有(?!\s*(?:一个|个)?问题)|我养了|我家有|\bmy\s+name\s+is\b|\bmy\s+birthday\s+is\b|\bi\s+am\b|\bi['’]?m\b|\bi\s+was\s+born\s+on\b|\bi\s+have\b|\bi\s+own\b)/i;
const NON_DURABLE_TOPIC_RE = /(我有\s*(?:一个|个)?问题|有个问题|报错|出现异常|exception|stack\s*trace)/i;
const PERSONAL_PROFILE_SIGNAL_RE = /(我叫|我是(?!谁|哪|几|多少)|我的名字是|我名字是|名字叫|我住在|我来自|我是做|我的职业|我的生日是|我生日是|我的出生日期是|我出生于|我生于|\bmy\s+name\s+is\b|\bi\s+am\b|\bi['’]?m\b|\bmy\s+birthday\s+is\b|\bmy\s+date\s+of\s+birth\s+is\b|\bi\s+was\s+born\s+on\b|\bi\s+was\s+born\s+in\b|\bi\s+live\s+in\b|\bi['’]?m\s+from\b|\bi\s+work\s+as\b)/i;
...
const QUESTION_BODY_RE = /(谁|什么|哪(?:里|儿|个|年|天|月)|几|多少|怎么|如何|为什么|为何)/u;
const QUESTION_TONE_SUFFIX_RE = /(啊|呀|哇)\s*$/u;
const PERSONAL_FACT_QUESTION_RE = /(我.*(?:是谁|叫什么|叫啥|哪年出生|什么时候出生|几岁|多少岁|生日是(?:什么|哪天)|出生日期是(?:什么|哪天|哪一年))|我的(?:生日|出生日期).*(?:是什么|是哪天|哪一年))/u;
...
export function isQuestionLikeMemoryText(text: string): boolean {
  ...
  if (PERSONAL_FACT_QUESTION_RE.test(normalized)) return true;
  if (QUESTION_BODY_RE.test(normalized) && QUESTION_TONE_SUFFIX_RE.test(normalized)) return true;
  return false;
}

function shouldKeepCandidate(text: string, isExplicit = false): boolean {
  const trimmed = normalizeText(text);
  if (!trimmed) return false;
  if (!isExplicit && trimmed.length < 6 && !SHORT_FACT_SIGNAL_RE.test(trimmed)) return false;
...
function extractExplicit(
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (!shouldKeepCandidate(raw, true)) continue;
...
function extractImplicit(options: ExtractTurnMemoryOptions): ExtractedMemoryChange[] {
  ...
  console.log('[Extract] Candidates:', candidates);
  ...
  for (const rawCandidate of candidates) {
    ...
    const keep = shouldKeepCandidate(candidate, false);
    if (!keep) {
        console.log(`[Extract] Rejected candidate: "${candidate}"`);
        continue;
    }
    ...
    if (NON_DURABLE_TOPIC_RE.test(candidate)) {
        console.log(`[Extract] Rejected (non-durable): "${candidate}"`);
        continue;
    }
    ...
    if (TRANSIENT_SIGNAL_RE.test(candidate)
      ...
      console.log(`[Extract] Rejected (transient): "${candidate}"`);
      continue;
    }
    ...
    if (confidence === 0) {
      console.log(`[Extract] Rejected (no signal match): "${candidate}"`);
      continue;
    }
    if (confidence < threshold) {
        console.log(`[Extract] Rejected (low confidence ${confidence} < ${threshold}): "${candidate}"`);
        continue;
    }
...
export function extractTurnMemoryChanges(options: ExtractTurnMemoryOptions): ExtractedMemoryChange[] {
  ...
  if (!userText) {
    console.log('[Extract] Empty user text');
    return [];
  }

  const explicitAdds = extractExplicit(userText, 'add', EXPLICIT_ADD_RE, 'explicit:add-command');
  if (explicitAdds.length) console.log('[Extract] Explicit Adds:', explicitAdds);

  const explicitDeletes = extractExplicit(userText, 'delete', EXPLICIT_DELETE_RE, 'explicit:delete-command');
  if (explicitDeletes.length) console.log('[Extract] Explicit Deletes:', explicitDeletes);

  const implicitAdds = extractImplicit(options);
  if (implicitAdds.length) console.log('[Extract] Implicit Adds:', implicitAdds);
