...
function isQuestionLikeMemoryText(text) {
    ...
    if (QUESTION_SUFFIX_RE.test(normalized))
        return true;
    if (PERSONAL_FACT_QUESTION_RE.test(normalized))
        return true;
    if (QUESTION_BODY_RE.test(normalized) && QUESTION_TONE_SUFFIX_RE.test(normalized))
        return true;
    return false;
}
function shouldKeepCandidate(text, isExplicit = false) {
    const trimmed = normalizeText(text);
    if (!trimmed)
        return false;
    if (!isExplicit && trimmed.length < 6 && !SHORT_FACT_SIGNAL_RE.test(trimmed))
        return false;
...
function extractExplicit(text, action, pattern, reason) {
    ...
    while ((match = pattern.exec(text)) !== null) {
        ...
        if (!shouldKeepCandidate(raw, true))
            continue;
...
function extractTurnMemoryChanges(options) {
    ...
    if (explicitAdds.length)
        console.log('[Extract] Explicit Adds:', explicitAdds);
    ...
    if (explicitDeletes.length)
        console.log('[Extract] Explicit Deletes:', explicitDeletes);
    const implicitAdds = extractImplicit(options);
    if (implicitAdds.length)
        console.log('[Extract] Implicit Adds:', implicitAdds);
    const merged = [];
    const seen = new Set();
...
//# sourceMappingURL=coworkMemoryExtractor.js.map